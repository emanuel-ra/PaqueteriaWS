...
namespace PaqueteriaWS.Controllers
{
    ...
    public class PaqueteExpressController : ApiController
    {
      [HttpGet]     
      public Reply Get(string zipcode)
      {
            ...
            {
                var data = (from d in db.PaqueteExpress where d.CodigoPostal == zipcode && d.cobertura != "ZONA PLUS" select d).ToList(); 
                if (data.Count > 0 ) 
                { 
                    reply.data = data;
